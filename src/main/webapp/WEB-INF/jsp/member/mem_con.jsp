...
<div class="container ">
	<div class="row">
		...
		<div class="col-md-9 ">
			...
			<div class="row post">
					...
					<c:if test="${!empty blist}">
						<c:forEach var="c" items="${blist}">
							<tr>
								...
								<td align="center"><a
									href="cm_cont.kkc?no=${c.cm_no}&page=${page}&state=cont">${c.cm_title}</a>
								</td>
				...
				<div id="table_search">
				<form method="get" action="mem_con.kkc" onsubmit="return find_check();">
  ...
  <c:if test="${(empty find_field) && (empty find_name)}">		
	...
	<c:if test="${page > 1 }">
	 <a href="mem_con.kkc?page=${page-1}">[이전]</a>&nbsp;
	</c:if>
	...
	<c:if test="${page < maxpage }">
	 <a href="mem_con.kkc?page=${page+1}">[다음]</a>
	</c:if>		
	</c:if>		
	...
	<c:if test="${page > 1 }">
	 <a href="mem_con.kkc?page=${page-1}&find_field=${find_field}&find_name=${find_name}">[이전]</a>&nbsp;
	</c:if>
	...
	<c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
	 ...
	 <c:if test="${a != page}">
	  <a href="mem_con.kkc?page=${a}&find_field=${find_field}&find_name=${find_name}">[${a}]</a>&nbsp;
	 </c:if>
	</c:forEach>
	<%-- 쪽번호 출력 --%>
	...
	<c:if test="${page < maxpage }">
	 <a href="mem_con.kkc?page=${page+1}&find_field=${find_field}&find_name=${find_name}">[다음]</a>
	</c:if>	
   </c:if>	
  </div>
				</div>
