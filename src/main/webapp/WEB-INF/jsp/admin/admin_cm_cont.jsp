...
<link rel="stylesheet" type="text/css" href="./admin_css/admin_cm/admin_cm_cont.css" /><%-- css적용 --%>
...
<div id="container_cm"><%-- 헤더아래 배경 --%>
 ...
 <div id="table_div">
  <table id="table" >
   ...
   <tr>
    <td class="td1">${adcb.cm_no}</td> 
    <td colspan="5" class="td1">${adcb.cm_title}</td>
   </tr>
   <tr>
    <th class="th1">작성자</th> <td class="td1">${adcb.cm_name}</td>
    <th class="th1">작성일</th> <td class="td1">${adcb.cm_date}</td>
    <th class="th1">조횟수</th> <td class="td1">${adcb.cm_hit}</td>
   </tr>
   <tr>
    <th colspan="6" class="th3">내용</th>
   </tr>
   <tr>
    <td class="td2" colspan="6">${adcb.cm_cont}</td>
   </tr>
  </table>
   <input type="button" value="삭제" class="btn1" 
    onclick="if(confirm('정말로 삭제할까요?')==true){location=
		'admin_cm_del.kkc?cm_no=${adcb.cm_no}&page=${page}&state=del';}else{ return; }" />
   <input type="button" value="수정" class="btn1"
	onclick="location='admin_cm_cont.kkc?cm_no=${adcb.cm_no}&page=${page}&state=edit';"/>
